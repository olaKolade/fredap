...
if ( !empty($args['size_px_mobile'])) {
	...
	$class_custom = 'jtx-header-mobile jtx-header-'.esc_attr($args['size_px_mobile']);
	wp_add_inline_style( 'aquaterias-theme-style', '.'.$class_custom.' { @media (max-width: 768px) { font-size: '.esc_attr($args['size_px_mobile']).' !important } } ' );
...
if ( !empty($args['subheader']) AND $args['style'] == 'head-subheader' ) {
	...
	if ($subtag == 'h6') $subclass_add = ' font-main'; else $subclass_add = '';
...
if ( !empty($args['image']) AND $args['icon_type'] != 'bg' ) {
	...
	$image = jtx_get_attachment_img_url( $args['image'] );
