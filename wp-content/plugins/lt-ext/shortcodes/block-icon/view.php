...
echo '<ul class="block-icon ' . esc_attr( $class ) .' ' . esc_attr($atts['type']).' align-' . esc_attr($atts['align']) . ' ' . esc_attr($atts['rounded']) . ' ' . esc_attr($atts['layout']) .'" '.$id.'>';
	foreach ( $atts['icons'] as $item ) {
		...
		if ($atts['type'] == 'icon-ht-right' OR $atts['type'] == 'icon-ht-left' OR $atts['layout'] == 'layout-inline') {
		...
		echo '<li'.$li_class.'>' . $href_tag1 . $image_tag . $href_tag2 . $div_tag1 . $item['header'] . '<div class="descr">'. esc_html( $item['descr'] ) . '</div>' . $div_tag2 . '</li>';
	}
